...
export async function processaPesquisaCep(input: EnderecoComPesquisa, bairroService: BairroService) {
...
export async function atualizarCoordenadasEndereco(enderecoId: string, enderecoService: ItemsService<Endereco>, provider: 'google' | 'mapbox', auth_token: string) {
    const endereco = await enderecoService.readOne(enderecoId, {
        fields: ['id', 'logradouro', 'numero', 'bairro.nome', 'bairro.cidade.nome', 'bairro.cidade.estado.nome'],
    });
